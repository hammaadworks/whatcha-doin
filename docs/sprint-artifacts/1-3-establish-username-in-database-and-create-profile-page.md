...
-   **Relevant architecture patterns and constraints:**
    ...
    -   **Reserved Slugs Enforcement:** A mechanism must be implemented to prevent `usernames` from conflicting with system routes (e.g., `auth`, `dashboard`). This should be considered when setting default usernames and validating user input for uniqueness.
    ...
    -   **Supabase Users Table Schema:** The `architecture.md` defines a `public_id` column (mapping to `username`) of type `text` with a `UNIQUE` constraint.
...
-   **Testing standards summary:**
    ...
    -   **Integration Tests:** Crucial for verifying RLS policies for the `public.users` table, ensuring the dev-mode mock user can `SELECT` and `UPDATE` their own `username` (`public_id`) correctly.
