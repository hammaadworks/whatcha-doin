...
export const HeroSection = () => {
  return (
    <section className="relative flex min-h-screen w-full flex-col items-center justify-center overflow-hidden py-24 md:py-32">
      ...
      <div className="pointer-events-none absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full max-w-[1000px] max-h-[1000px] opacity-100 z-0">
          {/* Inner Circle (Radius 120) */}
          <Suspense fallback={null}>
            <OrbitingCircles
                className="border-none bg-transparent" // Keep only styling not related to size
                iconSize={40}
                duration={25}
                delay={0}
                radius={120}
